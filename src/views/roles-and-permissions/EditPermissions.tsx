...
import { AllPermissions, PermissionsEnum } from "@utils/constants"
...
export default function EditUserPermissions({ open = true, onClose = () => null, onSave, creatinguser, permissions, setIsAdmin }: AddUserPermissionsProps) {
            ...
            permissions: [
                {
                    ...
                    enabled: permissions?.includes(PermissionsEnum.BROADCAST_MESSAGE_SEND_BROADCAST) ? true : false,
                },
            ],
        },
        {
            ...
            permissions: [
                {
                    ...
                    enabled: permissions?.includes(PermissionsEnum.INDIVIDUAL_MESSAGE_SEND_MESSAGE) ? true : false,
                },
            ],
        },
        {
            ...
            permissions: [
                {
                    ...
                    enabled: permissions?.includes(PermissionsEnum.INBOX_READ_ALL_CHATS) ? true : false,
                },
                {
                    ...
                    enabled: permissions.includes(PermissionsEnum.INBOX_SEND_MESSAGE) ? true : false,
            ...
            permissions: [
                {
                    ...
                    enabled: permissions.includes(PermissionsEnum.ROLES_AND_PERMISSIONS_VIEW_USERS) ? true : false,
                },
                {
                    id: PermissionsEnum.ROLES_AND_PERMISSIONS_EDIT_USER,
                    label: "Update User",
                    description: "Allows the user to access and edit user details.",
                    enabled: permissions.includes(PermissionsEnum.ROLES_AND_PERMISSIONS_EDIT_USER) ? true : false,
                ...
                },
                {
                    ...
                    enabled: permissions.includes(PermissionsEnum.ROLES_AND_PERMISSIONS_CREATE_USER) ? true : false,
                    dependencies: [PermissionsEnum.ROLES_AND_PERMISSIONS_VIEW_USERS],
                },
                {
                    ...
                    enabled: permissions.includes(PermissionsEnum.ROLES_AND_PERMISSIONS_ACTIVE_INACTIVE_USER) ? true : false,
                    dependencies: [PermissionsEnum.ROLES_AND_PERMISSIONS_VIEW_USERS],
                },
            ],
        },
    ...
    const [expandedModules, setExpandedModules] = useState<Record<number, boolean>>({
        0: permissions?.includes(PermissionsEnum.BROADCAST_MESSAGE_MODULE) ? true : false,
        1: permissions?.includes(PermissionsEnum.INDIVIDUAL_MESSAGE_MODULE) ? true : false,
        2: permissions?.includes(PermissionsEnum.INBOX_MODULE) ? true : false,
        3: permissions?.includes(PermissionsEnum.ROLES_AND_PERMISSIONS_MODULE) ? true : false,
    })
    ...
    // Handle save
    const handleSave = () => {
        onSave(selectedPermissionsFunc())
        if (!open) {
            onClose()
        }
    }
    ...
    useEffect(() => {
        if (setIsAdmin && permissionModules) {
            selectedPermissionsFunc().length === AllPermissions.length ? setIsAdmin(true) : setIsAdmin(false);
        }
            ...
            <>
                <Box sx={{ overflowY: "auto", height: "100%" }}>
                    {permissionModules.map((module, moduleIndex) => (
                        <Box key={module.id} className={styles.permissionItem}>
                            <Box
